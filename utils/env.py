...
class CustomMultiTaskRLBenchEnv(MultiTaskRLBenchEnv):
    ...
    def reset_to_demo(self, i, variation_number=-1, start_new=False):
        if self._episodes_this_task == self._swap_task_every or start_new:
            self._set_new_task()
            self._episodes_this_task = 0
        self._episodes_this_task += 1
    ...
    def get_color_information(self):
        """ directly use the color information in rlbench to avoid the work of building an external color detector or collecting
        demonstrations for all colors 
        """
        task = self._task._scene.task
        return {
            'target': getattr(task, '_color_target', None),
            'distractors': getattr(task, '_color_distractors', [])
        }
